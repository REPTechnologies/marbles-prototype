(function( $ ){
  ...
  var methods = {
    init: function(options) {
      ...
      this.css({height: width + 'px', width: '85%',
                background: '#A7A7C4', position: 'relative',
                '-webkit-border-radius': '2em', '-moz-border-radius': '2em'});   
    
      
      var smiley = $('<div class="smileyslider-smiley"></div>');
      smiley.css({cursor: 'pointer', width: width + 'px', height: width + 'px', position: 'relative'});
      this.append(smiley);
      this.data('smiley', smiley);

      var paper = Raphael(smiley[0], width, width);
      var face = paper.circle(width/2, width/2, (width/2-5)).attr("fill", "#FF6600").attr("stroke", "#000");
